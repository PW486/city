        ...
        <div className="grid gap-4">
          {filteredAndSortedCities.length > 0 ? (
            filteredAndSortedCities.map((city, index) => (
              ...
              >
                <div className="flex flex-col lg:flex-row items-start lg:items-center gap-5">
                  {/* Ranking Badge */}
                  <div className="flex-shrink-0 w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black text-lg">
                    {index + 1}
                  </div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full lg:w-auto">
                    <ScoreItem icon={<CreditCard className="w-3.5 h-3.5 text-amber-500" />} label="Rent" value={`$${city.rentPrice1BR}`} active={sortBy === 'rent'} />
                    <ScoreItem icon={<Shield className="w-3.5 h-3.5 text-blue-500" />} label="Safety" value={city.safetyIndex} active={sortBy === 'safety'} />
                    <ScoreItem icon={<Users className="w-3.5 h-3.5 text-indigo-500" />} label="Expat" value={city.expatIndex} active={sortBy === 'expat'} />
                    
                    <div className={`flex flex-col items-center justify-center rounded-xl px-4 py-2 border-2 transition-all ${sortBy === 'total' ? 'bg-emerald-600 border-emerald-600 text-white shadow-md' : 'bg-slate-50 border-slate-100 text-slate-900'}`}>
                      <span className={`text-[8px] uppercase font-black tracking-widest mb-0.5 ${sortBy === 'total' ? 'text-emerald-100' : 'text-slate-400'}`}>Total</span>
                ...
                <div className="grid sm:grid-cols-3 gap-4 pt-5 mt-5 border-t border-slate-100">
                  <ProgressMini label="Rent Score" score={city.rentScore} color="bg-amber-500" />
                  <ProgressMini label="Safety Score" score={city.safetyIndex} color="bg-blue-500" />
                  <ProgressMini label="Expat Score" score={city.expatIndex} color="bg-indigo-500" />
                </div>
              </div>
            ))
          ) : (
            <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
              ...
              <button onClick={() => setSearchQuery('')} className="mt-4 px-5 py-2.5 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-emerald-600 transition-colors">Clear search</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <button
      ...
      className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
        active ? 'bg-white text-slate-900 shadow-sm scale-105' : 'text-slate-500 hover:text-slate-700'
      }`}
    >
      {label}
    </button>
...
function ScoreItem({ icon, label, value, active }: { icon: React.ReactNode, label: string, value: string | number, active: boolean }) {
  return (
    <div className={`flex flex-col items-center justify-center p-2 rounded-xl border transition-all ${active ? 'bg-slate-900 border-slate-900 shadow-sm scale-105' : 'bg-white border-slate-50'}`}>
      <div className={`flex items-center gap-1 text-[8px] uppercase font-black tracking-widest mb-1 ${active ? 'text-slate-400' : 'text-slate-400'}`}>
        {icon} {label}
      </div>
      <div className={`text-sm font-black ${active ? 'text-white' : 'text-slate-900'}`}>{value}</div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
        <div className={`h-full ${color} transition-all duration-700`} style={{ width: `${score}%` }}></div>
      </div>
    </div>
  )
}
